...
for the Ascend ngram proposer hot-path optimization.
...
3. The Ascend-side overhead is comparable to the upstream vLLM ngram path.
...
Measure the proposer-only overhead:
...
Measure the runner-side integration overhead:
...
At the time of writing, the optimized Ascend path is in the same latency band
as upstream vLLM for the small CPU-only benchmark above, and should not be
materially slower in proposer-only or runner-side integration overhead.
